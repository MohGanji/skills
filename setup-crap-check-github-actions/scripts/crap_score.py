...
_FUNC_PATTERNS: Dict[str, List[str]] = {
    "js": [
        ...
        r"^\s+(\w+)\s*:\s*(?:async\s+)?(?:\([^)]*\)|[a-zA-Z_]\w*)\s*=>\s*[\({]",
    ],
    "java": [
...
# ---------------------------------------------------------------------------
# .crapignore support (gitignore-like exclusion)
# ---------------------------------------------------------------------------

def _load_crapignore(base: Path) -> List[Tuple[bool, str]]:
    """Load .crapignore from base directory. Returns [(negated, pattern), ...]."""
    path = base / ".crapignore"
    if not path.is_file():
        return []
...
def _is_crapignored(rel_path: str, rules: List[Tuple[bool, str]]) -> bool:
    """Check if a relative path should be ignored based on .crapignore rules."""
    if not rules:
        return False
    ignored = False
    ...
    for neg, pattern in rules:
        pat = pattern.rstrip("/")
        if "/" in pat:
            # Path-based pattern -- match against full relative path
            match = fnmatch.fnmatch(rel, pat)
        else:
            # Name-only pattern -- match filename or any directory component
            match = fnmatch.fnmatch(name, pat) or any(
...
def main():
    ...
    # load .crapignore rules
    ignore_rules = _load_crapignore(base)
    ...
    # apply .crapignore filtering
    if ignore_rules:
        filtered: List[str] = []
        for fp in src_files:
            try:
                rel = str(Path(fp).resolve().relative_to(base))
            except ValueError:
                rel = fp
            if not _is_crapignored(rel, ignore_rules):
                filtered.append(fp)
        src_files = filtered
